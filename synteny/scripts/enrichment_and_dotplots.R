# enrichment_and_dotplots.R -- pipeline stage 3a (of 3).
#
# For each pairwise "raw" synteny table produced by prep_synteny.py
# (data/synteny_tables/raw/*_synt.txt), test each chromosome x CLG cell for
# significant enrichment (Fisher's exact test, Bonferroni-corrected, see
# testEnrich() in synteny_functions.R), and write the annotated table
# (data/synteny_tables/annotated/*_syntr.txt, with an extra `scol` column:
# 'S' = significant, 'NS' = not) that feeds ideogram_figures.R.
#
# Also produces exploratory dotplots: one PNG per best-matching chromosome
# pair for the Mespilia globulus blue-vs-red comparison
# (figures/per_chromosome_pair/blue_vs_red_Mglob/), and one CLG-coloured,
# clustering-ordered dotplot per species pair (rendered to the active
# graphics device -- run interactively or wrap calls in pdf()/png() as
# needed).
#
...
source("synteny_functions.R")

RAW_DIR <- "../data/synteny_tables/raw"
ANNOT_DIR <- "../data/synteny_tables/annotated"
FIG_DIR <- "../figures/per_chromosome_pair/blue_vs_red_Mglob"

# CLG (conserved/ancestral linkage group) colour palette, shared with
# ideogram_figures.R -- keep the two in sync if edited.
...
### ---- Mespilia globulus: blue morph vs red morph (sex-linked chr4 context, see ../../sex_determination/) ----
#by position by chromosomes
bmglob_rmglob_synt <- read.table(file.path(RAW_DIR, "mglob_blue_vs_red_synt.txt"), sep='\t', h=T)
...
for(i in 1:nrow(best_pairs)) {
  ...
  if(nrow(chr_data) >= 1) {
    # Create filename
    filename <- file.path(FIG_DIR, paste0(chr1, "_vs_", chr2, ".png"))
...
write.table(bmglob_rmglob_synt.exp,file=file.path(ANNOT_DIR, "mglob_blue_vs_red_syntr.txt"),sep='\t',quote=F,row.names=F)

### ---- Holothuria leucospilota (outgroup) vs M. globulus ----
hleu_mglob_synt=read.table(file.path(RAW_DIR, "hleu_vs_mglob_synt.txt"),sep='\t',h=T)
...
write.table(hleu_mglob_synt.exp,file=file.path(ANNOT_DIR, "hleu_vs_mglob_syntr.txt"),sep='\t',quote=F,row.names=F)

### ---- M. globulus vs Paracentrotus lividus ----
mglob_pliv_synt=read.table(file.path(RAW_DIR, "mglob_vs_pliv_synt.txt"),sep='\t',h=T)
...
write.table(mglob_pliv_synt.exp,file=file.path(ANNOT_DIR, "mglob_vs_pliv_syntr.txt"),sep='\t',quote=F,row.names=F)

### ---- P. lividus vs Strongylocentrotus purpuratus ----
pliv_spurp_synt=read.table(file.path(RAW_DIR, "pliv_vs_spur_synt.txt"),sep='\t',h=T)
...
write.table(pliv_spurp_synt.exp,file=file.path(ANNOT_DIR, "pliv_vs_spur_syntr.txt"),sep='\t',quote=F,row.names=F)

### ---- S. purpuratus vs Lytechinus pictus ----
spurp_lpict_synt=read.table(file.path(RAW_DIR, "spur_vs_lpic_synt.txt"),sep='\t',h=T)
...
write.table(spurp_lpict_synt.exp,file=file.path(ANNOT_DIR, "spur_vs_lpic_syntr.txt"),sep='\t',quote=F,row.names=F)

### ---- L. pictus vs Lytechinus variegatus ----
lpict_lvar_synt=read.table(file.path(RAW_DIR, "lpic_vs_lvar_synt.txt"),sep='\t',h=T)
...
write.table(lpict_lvar_synt.exp,file=file.path(ANNOT_DIR, "lpic_vs_lvar_syntr.txt"),sep='\t',quote=F,row.names=F)

### ---- L. variegatus vs L. pictus (reciprocal direction, separate MBH run) ----
lvar_lpict_synt=read.table(file.path(RAW_DIR, "lvar_vs_lpic_synt.txt"),sep='\t',h=T)
...
write.table(lvar_lpict_synt.exp,file=file.path(ANNOT_DIR, "lvar_vs_lpic_syntr.txt"),sep='\t',quote=F,row.names=F)

### ---- S. purpuratus vs L. variegatus ----
spurp_lvar_synt=read.table(file.path(RAW_DIR, "spur_vs_lvar_synt.txt"),sep='\t',h=T)
...
write.table(spurp_lvar_synt.exp,file=file.path(ANNOT_DIR, "spur_vs_lvar_syntr.txt"),sep='\t',quote=F,row.names=F)
