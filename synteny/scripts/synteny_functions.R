# synteny_functions.R -- shared helper library for macrosynteny plotting and
# statistics. Sourced by enrichment_and_dotplots.R; not run directly.
#
# Expects a "synt" data frame as produced by prep_synteny.py / macrosynteny.py,
# with columns s1g,s1gp,s1chr,s1gi,s2g,s2gp,s2chr,s2gi[,clg] (species
# 1/2 gene id, position, chromosome, rank index, and CLG label).
#
# Contents:
#   plotSynt / plotSynts        - raw position dotplots (s1 vs s2), by chromosome or CLG colour
#   plotSyntCLGr / plotSyntCLGrs / plotSyntCLGoc - rank-index dotplots coloured by CLG
#   ordSynt / ordSyntRw / ordSyntJO - reorder chromosomes by hierarchical clustering
#                                      of the s1chr x s2chr contingency table (pheatmap)
#   testEnrichCLG / testEnrichCLG1 / testEnrich - Fisher's exact test for
#                                      significant enrichment of a CLG (or
#                                      chromosome) on a given chromosome, with
#                                      Bonferroni correction (scol: 'S'/'NS')
#   gchkExp / gchkExpCLG         - windowed (20-gene) composition summaries for
#                                      alluvial/ribbon-style enrichment plots
