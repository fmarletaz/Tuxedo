# piechart_cafe.R -- pipeline stage 3 (of 3), final figure.
#
# Plots the time-calibrated species tree with a pie chart at each node/tip
# showing the proportion of significantly expanded / contracted / unchanged
# gene families on that branch (from total_sig_exp_contr_summary.csv, the
# small+large-family CAFE runs combined -- see extract_sig_exporcontr.py's
# header for the caveat that this combination step itself isn't captured
# here), plus the raw E/C/NC counts printed under each tip label.
#
# Run from inside scripts/ (paths below are relative to this file).
#
...
# ---------- CONFIG ----------
tree_file <- "../data/species_tree/SpeciesTree_rooted_time_cal.txt"
csv_file  <- "../data/cafe/output_small_families/total_sig_exp_contr_summary.csv"
pdf_out   <- "../figures/tree_node_pies.pdf"
svg_out   <- "../figures/tree_node_pies.svg"
