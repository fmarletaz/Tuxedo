"""
tissue_heatmap.py -- final figure, gene-family tissue-expression heatmap.

For each candidate gene family, maps its Mglob orthogroup members to gene
IDs (via OrthoFinder's Orthogroups.tsv), pulls each gene's tissue TPMs from
../data/expression/Mglob_gathered_tpms_tissues.txt, sums isoforms to the
gene level then genes to the family level, and plots a row-z-scored
log2(TPM+1) heatmap by tissue. Output: ../figures/.

CAVEAT: orthogroups_file below (Orthogroups.tsv, OrthoFinder's raw
orthogroup-membership table) was not included in this archive -- this
script cannot currently be run without it.
"""
...
raw_counts_file = Path("../data/copy_number/gene_family_raw_counts.csv")
orthogroups_file = Path("Orthogroups.tsv")  # not present in this archive, see CAVEAT above
tpm_file = Path("../data/expression/Mglob_gathered_tpms_tissues.txt")

output_svg = Path("../figures/mglob_gene_family_expression_heatmap.svg")
output_matrix = Path("../figures/mglob_gene_family_expression_heatmap_matrix.csv")
