...
### hleu vc mglob 
lg=read.table("hleu_mglob_syntr.txt",h=T)
...
#k_mglob_ord=c("scaffold_16","scaffold_17","scaffold_1", "scaffold_4", "scaffold_2", "scaffold_7",
#              "scaffold_12", "scaffold_6", "scaffold_8", "scaffold_11", "scaffold_3",
#              "scaffold_19", "scaffold_14", "scaffold_9", "scaffold_13",
#              "scaffold_18", "scaffold_15","scaffold_5", "scaffold_10")
...
ideogram(karyotype = chroms, synteny = lgt, output = 'hleu_mglob_syntr_ideogram.svg' )
convertSVG("hleu_mglob_syntr_ideogram.svg", device = "png")


#mglob vs pliv
lg=read.table("mglob_pliv_syntr.txt",h=T)
lg %>%  group_by(s1chr) %>%   summarise(End=max(s1gp)) %>% arrange(-End) %>% rename(Chr=s1chr) %>% 
...
ideogram(karyotype = chroms, synteny = lgt, output = 'mglob_pliv_syntr_ideogram.svg' )
convertSVG("mglob_pliv_syntr_ideogram.svg", device = "png")
###### 3. PLIV vs SPUR ######
lg=read.table("pliv_spurp_syntr.txt",h=T)
lg %>%  group_by(s1chr) %>%   summarise(End=max(s1gp)) %>% arrange(-End) %>% rename(Chr=s1chr) %>% 
...
ideogram(karyotype = chroms, synteny = lgt, output = 'pliv_spur_syntr_ideogram.svg' )
convertSVG("pliv_spur_syntr_ideogram.svg", device = "png")

###### 4. SPUR vs LPIC ######
lg=read.table("spurp_lpict_syntr.txt",h=T)
lg %>%  group_by(s1chr) %>%   summarise(End=max(s1gp)) %>% arrange(-End) %>% rename(Chr=s1chr) %>% 
  mutate(Start=1,species='purpuratus',fill="purpuratus",size=12,color=252525) %>% relocate(Start,.after=Chr)-> k_spur
lg %>%  group_by(s2chr) %>%   summarise(End=max(s2gp)) %>% arrange(-End) %>% rename(Chr=s2chr) %>% 
...
lg %>% filter(scol=='S') %>% select(Species_1,s1gp,Species_2,s2gp,clg) %>% rename(Start_1=s1gp,Start_2=s2gp) %>%
  ...
  relocate(End_1,.after=Start_1) %>% relocate(End_2,.after=Start_2) %>% select(-clg) -> lgt
lgt <- na.omit(lgt)
ideogram(karyotype = chroms, synteny = lgt, output = 'spur_lpic_syntr_ideogram.svg' )
convertSVG("spur_lpic_syntr_ideogram.svg", device = "png")

###### 5. LPIC vs LVAR ######
lg=read.table("lpict_lvar_syntr.txt",h=T)
lg %>%  group_by(s1chr) %>%   summarise(End=max(s1gp)) %>% arrange(-End) %>% rename(Chr=s1chr) %>% 
  mutate(Start=1,species='pictus',fill="pictus",size=12,color=252525) %>% relocate(Start,.after=Chr)-> k_lpic
lg %>%  group_by(s2chr) %>%   summarise(End=max(s2gp)) %>% arrange(-End) %>% rename(Chr=s2chr) %>% 
...
lg %>% filter(scol=='S') %>% select(Species_1,s1gp,Species_2,s2gp,clg) %>% rename(Start_1=s1gp,Start_2=s2gp) %>%
  ...
  relocate(End_1,.after=Start_1) %>% relocate(End_2,.after=Start_2) %>% select(-clg) -> lgt
lgt <- na.omit(lgt)
ideogram(karyotype = chroms, synteny = lgt, output = 'lpic_lvar_syntr_ideogram.svg' )
convertSVG("lpic_lvar_syntr_ideogram.svg", device = "png")

###### 6. Spurp vs LVAR ######
lg=read.table("spurp_lvar_syntr.txt",h=T)
lg %>%  group_by(s1chr) %>%   summarise(End=max(s1gp)) %>% arrange(-End) %>% rename(Chr=s1chr) %>% 
  mutate(Start=1,species='purpuratus',fill="purpuratus",size=12,color=252525) %>% relocate(Start,.after=Chr)-> k_spur
lg %>%  group_by(s2chr) %>%   summarise(End=max(s2gp)) %>% arrange(-End) %>% rename(Chr=s2chr) %>% 
  mutate(Start=1,species='variegatus',fill="variegatus",size=12,color=252525) %>% relocate(Start,.after=Chr)-> k_lvar
...
lg %>% filter(scol=='S') %>% select(Species_1,s1gp,Species_2,s2gp,clg) %>% rename(Start_1=s1gp,Start_2=s2gp) %>%
  ...
  relocate(End_1,.after=Start_1) %>% relocate(End_2,.after=Start_2) %>% select(-clg) -> lgt
lgt <- na.omit(lgt)
ideogram(karyotype = chroms, synteny = lgt, output = 'spur_lvar_syntr_ideogram.svg' )
convertSVG("spur_lvar_syntr_ideogram.svg", device = "png")

###### 7. LVAR vs LPIC ######
lg=read.table("lvar_lpict_syntr.txt",h=T)
lg %>%  group_by(s1chr) %>%   summarise(End=max(s1gp)) %>% arrange(-End) %>% rename(Chr=s1chr) %>% 
  mutate(Start=1,species='variegatus',fill="variegatus",size=12,color=252525) %>% relocate(Start,.after=Chr)-> k_lvar
lg %>%  group_by(s2chr) %>%   summarise(End=max(s2gp)) %>% arrange(-End) %>% rename(Chr=s2chr) %>% 
  mutate(Start=1,species='pictus',fill="pictus",size=12,color=252525) %>% relocate(Start,.after=Chr)-> k_lpic
...
lg %>% filter(scol=='S') %>% select(Species_1,s1gp,Species_2,s2gp,clg) %>% rename(Start_1=s1gp,Start_2=s2gp) %>%
  ...
  relocate(End_1,.after=Start_1) %>% relocate(End_2,.after=Start_2) %>% select(-clg) -> lgt
lgt <- na.omit(lgt)
ideogram(karyotype = chroms, synteny = lgt, output = 'lvar_lpic_syntr_ideogram.svg' )
convertSVG("lvar_lpic_syntr_ideogram.svg", device = "png")

###### 7. blue vs red ######
lg=read.table("bmglob_rmglob_syntr.txt",h=T)
lg %>%  group_by(s1chr) %>%   summarise(End=max(s1gp)) %>% arrange(-End) %>% rename(Chr=s1chr) %>% 
...
lg %>% filter(scol=='S') %>% select(Species_1,s1gp,Species_2,s2gp,clg) %>% rename(Start_1=s1gp,Start_2=s2gp) %>%
  ...
  relocate(End_1,.after=Start_1) %>% relocate(End_2,.after=Start_2) %>% select(-clg) -> lgt
lgt <- na.omit(lgt)
ideogram(karyotype = chroms, synteny = lgt, output = 'bmglob_rmglob_syntr_ideogram.svg' )
convertSVG("bmglob_rmglob_syntr_ideogram.svg", device = "png")
